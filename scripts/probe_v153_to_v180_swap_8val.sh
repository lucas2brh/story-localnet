#!/usr/bin/env bash
# probe_v153_to_v180_swap_8val.sh
#
# Scenario A-v180 of v1.8.0 binary-swap-test-plan.md: all 8 vals on v1.5.3 → v1.8.0 rolling swap.
# Verifies upstream/hans/impl-1.8 head (= main + Seneca, PR piplabs/story#835) executes Seneca
# correctly when chain was bootstrapped on v1.5.3 (mainnet majority binary).
#
# Topology (single source — mainnet truth, no DKG cohort like v1.7.0 Aeneid path):
#   val1..val8 = story-node:v1.5.3-localnet (mainnet majority operator binary)
#   bootnode1  = story-node:v1.5.3-localnet (non-validator)
#
# Heights (in hans/impl-1.8 patched upgrades.go for StoryLocalnetID):
#   Horace = 10  — fires pre-swap on v1.5.3 binary (its own handler version)
#   H_swap = 30  — stop+restart each validator to v1.8.0, rolling 1 at a time
#   Seneca = 100 — applyDeferredMaxValidatorsChange fires on v1.8.0 binary (NEW path)
...
#   A.1 v1.8.0 "Applied deferred MaxValidators reduction" log on all 8 nodes at H=100
...
#   A.7 Pre-Seneca symmetry: at H=99 (one block pre-Seneca, all 8 on v1.8.0), all 8 nodes return identical app_hash
#       (trivial in single-source topology, but kept for parity with v1.7.0 probe and as final sanity gate)
#
# Usage:
#   ./scripts/probe_v153_to_v180_swap_8val.sh
#   SKIP_TEARDOWN=1 ./scripts/probe_v153_to_v180_swap_8val.sh    # leave chain running after
#   SKIP_BOOT=1 ...                                              # only if you know chain is clean v1.5.3 state
#
# Outputs:
#   tmp/probe-v180-binary-swap-evidence/  during run
#   docs/test-evidence/v180-binary-swap-paths-<date>/A-v153-to-v180/  after copy
...
EV_DIR="${LOCALNET}/tmp/probe-v180-binary-swap-evidence"
...
# verify 2 docker images exist
for v in v1.5.3 v1.8.0; do
...
if [ "$SKIP_BOOT" != "1" ]; then
  ...
  docker compose \
    ...
    -f "$LOCALNET/compose.scenario-A-v180.yml" \
...
for N in 1 2 3 4 5 6 7 8; do
  ...
  [ "$IMG" = "story-node:v1.5.3-localnet" ] || fail "val${N} image=$IMG, want story-node:v1.5.3-localnet"
done
log "✓ topology: val1..val8 all v1.5.3 (single source)"
...
[ "$DISTINCT" = "1" ] || fail "A.7 pre-swap divergence — unexpected with single-source v1.5.3 topology"
log "✓ A.7 pre-swap consistency passed (trivial with single-source)"

# --- Phase 2: rolling swap to v1.8.0 ---
...
# we verify all 8 actually on v1.8.0 at end of loop (chain-asserted).
...
for N in 1 2 3 4 5 6 7 8; do
  ...
  docker compose \
    ...
    -f "$LOCALNET/compose.scenario-A-v180-post-swap.yml" \
    stop "validator${N}-node" >/dev/null 2>&1
  log "swap val${N}: up (v1.8.0 image)"
  docker compose \
    -f "$LOCALNET/docker-compose-validator${N}.yml" \
    -f "$LOCALNET/compose.scenario-A-v180-post-swap.yml" \
...
# verify all 8 actually on v1.8.0 (chain-asserted via per-container story binary)
# NOTE: `story version` writes to STDERR, not stdout. Must use 2>&1 to capture.
log "verifying all 8 validators report v1.8.0-stable"
not_v180=0
for N in 1 2 3 4 5 6 7 8; do
  ...
  if [ "$V" != "v1.8.0-stable" ]; then
    not_v180=$((not_v180 + 1))
  fi
done
[ "$not_v180" = "0" ] || fail "$not_v180 validators not on v1.8.0 after swap (see $EV_DIR/swap-version-check.txt)"
log "✓ all 8 validators on v1.8.0-stable"

# --- Phase 3: post-swap pre-Seneca state-hash at H_POST_SWAP=35 ---
wait_until_height "$H_POST_SWAP"
log "dumping AppHash at H=$H_POST_SWAP (5 blocks post-swap, all on v1.8.0)"
...
# --- Phase 4: pre-Seneca consistency check at H=H_PRE_SENECA=99 (A.7) ---
wait_until_height "$H_PRE_SENECA"
log "dumping AppHash at H=$H_PRE_SENECA (one block pre-Seneca)"
DISTINCT=$(dump_apphash_all_vals "$H_PRE_SENECA" "$EV_DIR/A7-pre-seneca-H${H_PRE_SENECA}-apphash.txt")
log "distinct: $DISTINCT (want 1)"
[ "$DISTINCT" = "1" ] || fail "A.7 pre-Seneca consistency failed at H=$H_PRE_SENECA"
log "✓ A.7 pre-Seneca consistency passed"
...
log ""
log "Pre-swap H=$H_PRE_SWAP: 1 distinct hash → ✓ A.7"
log "Post-swap H=$H_POST_SWAP: 1 distinct hash → ✓ A.6"
log "Pre-Seneca H=$H_PRE_SENECA: 1 distinct hash → ✓ A.7"
