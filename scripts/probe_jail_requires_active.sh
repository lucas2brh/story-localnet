...
phase_0_start() {
  ...
  MAX_VALIDATORS_INIT=4 STORY_BIN="$STORY_BIN" bash "${LOCALNET}/scripts/assemble_genesis.sh" 20 2>&1 | tail -1
  local mv_set
...
phase_3_self_redelegate() {
  ...
  # 50% of original self-del (TARGET_TOKENS_BASELINE) in stake-units, *1e9 → wei
  local amount_wei
  amount_wei=$(echo "$TARGET_TOKENS_BASELINE / 2 * 1000000000" | bc)
  log "  redelegate amount = 50% of $TARGET_TOKENS_BASELINE = $amount_wei wei"
